...
test.serial('POST `/api/places` creates a new place', async (t) => {
  ...
  t.is(response.name, testPlacePayload.name, 'response contains created place');

  const queriedPlace = await Place.findOne({ _id: response._id });
...
test.serial('PATCH `/api/places/:id` updates a place', async (t) => {
  ...
  t.is(updates.name, response.name, 'response shows updates');

  const queriedUser = await Place.findOne({ _id: response._id });
  t.is(queriedUser.name, response.name, 'database shows updates');
});
test.serial('GET `/api/places/:id` gets a place', async (t) => {
  ...
  t.is(response.name, testPlacePayload.name, 'response shows the updates');
